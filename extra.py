...
def replace_encoded(message: str): # Replaces special encoded characters to their actual characters
    return message.replace('%21', '!').replace('%40', '@').replace('%23', '#').replace('%24', '$').replace('%5E', '^').replace('%26', '&').replace('%28', '(').replace('%29', ')').replace('%2D', '-').replace('%5F', '_').replace('%3D', '=').replace('%3F', '?').replace('%2B', '+').replace('+', ' ').replace('%25', '%')
